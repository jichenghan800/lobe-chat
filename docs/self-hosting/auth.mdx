...
The label will be automatically prefixed with "Continue with ", so `AUTH_OKTA_LABEL="Corporate SSO"` will display as "Continue with Corporate SSO" on the login page.

<Callout type={'tip'}>
  Provide only the provider name in the label (e.g., "Corporate SSO", not "Continue with Corporate SSO"). The "Continue with " prefix is added automatically.
</Callout>
