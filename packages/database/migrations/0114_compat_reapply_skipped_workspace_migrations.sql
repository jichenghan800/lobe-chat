-- Compatibility migration for long-lived Cotti environments whose local
-- custom migration timestamps are newer than upstream 0106~0109. Drizzle skips
-- those upstream workspace migrations in that state, so replay their guarded
-- DDL here under a newer migration timestamp.
