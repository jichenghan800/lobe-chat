...
describe('computeMessageCostSplit', () => {
  it('preserves an explicitly recorded zero instead of estimating a positive charge', () => {
    const split = computeMessageCostSplit(
      { totalInputTokens: 1_000_000, totalOutputTokens: 1_000 },
      priced.providerId,
      priced.id,
      0,
    );
    expect(split.totalCost).toBe(0);
    expect(split.inputCost + split.cachedInputCost + split.cacheWriteCost + split.outputCost).toBe(
      0,
    );
  });
  ...
  it('selects the long-context tier and skips an unpriced Azure model fallback', () => {
    const split = computeMessageCostSplit(
      { totalInputTokens: 280_000, totalOutputTokens: 1_000 },
      'azure',
      'gpt-5.6-sol',
    );
