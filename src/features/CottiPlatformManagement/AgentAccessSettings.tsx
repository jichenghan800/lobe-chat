...
import { shouldConfirmAgentAccessModeChange } from './peopleManagementSafety';
...
const AgentAccessSettings = memo(() => {
  ...
  const applyMode = async (nextMode: CottiAgentAccessMode) => {
  ...
  const handleSetMode = (nextMode: CottiAgentAccessMode) => {
    if (nextMode === mode) return;

    if (shouldConfirmAgentAccessModeChange(mode, nextMode)) {
      confirmModal({
        content: t('platformManagement.agentAccess.mode.open.confirm.content'),
        okButtonProps: { danger: true },
        okText: t('platformManagement.agentAccess.mode.open.confirm.action'),
        onOk: () => void applyMode(nextMode),
        title: t('platformManagement.agentAccess.mode.open.confirm.title'),
      });
      return;
    }
  ...
  const handleRemoveRule = useCallback(
    (id: string) => {
      confirmModal({
        content: t('platformManagement.agentAccess.deleteConfirm'),
        okButtonProps: { danger: true },
        ...
        title: t('platformManagement.agentAccess.actions.delete'),
      });
    },
    [detailSWR, t],
  );
    ...
    <Flexbox gap={16}>
      <Block className={sharedStyles.card} gap={12} padding={20} variant={'outlined'}>
        ...
        <Segmented
          ...
          onChange={(value) => handleSetMode(value as CottiAgentAccessMode)}
        />
        <Text className={sharedStyles.copy} fontSize={12}>
