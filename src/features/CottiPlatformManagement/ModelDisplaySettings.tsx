...
import { openModelRetirementModal } from './ModelRetirementModal';
...
const ModelDisplayEditor = memo(
  ({ groupId, onBusyChange }: { groupId?: string; onBusyChange: (busy: boolean) => void }) => {
    const { t } = useTranslation('setting');
    const [scope, setScope] = useState<ModelDisplayScope>('chat');
    const [selectedOptionKey, setSelectedOptionKey] = useState<string>();
    const configSWR = useClientDataSWR(
      ['cotti', 'admin-model-display-config', groupId ?? 'default'],
      () => cottiModelDisplayService.getAdminConfig(groupId),
      { revalidateOnFocus: true },
    );
    const optionsSWR = useClientDataSWR(
      ['cotti', 'model-display-options', groupId ?? 'default'],
      () => cottiModelDisplayService.getOptions(groupId),
      { revalidateOnFocus: false },
    );
    const { draft, saving, status, error, retry, save } = useConfigAutosave(
      configSWR.data,
      async (next) => {
        const saved = await cottiModelDisplayService.updateConfig(next, groupId);
    ...
    const onMigrationSaved = async (config: ModelDisplayConfig) => {
        ...
        <Block className={sharedStyles.card} gap={16} padding={20} variant={'outlined'}>
          <Flexbox className={sharedStyles.sectionHeader} gap={4}>
            <Text className={sharedStyles.sectionTitle}>
              {t('platformManagement.models.title')}
            </Text>
          ...
          {draft[scope].length === 0 ? (
            ...
            <div className={styles.modelList}>
              {draft[scope].map((item, index) => {
                ...
                const isProfessionalChannel = !groupId && isCottiProfessionalChannel(item);
                ...
                return (
                  <div className={styles.modelRow} key={getModelDisplayKey(item)}>
                    ...
                    <Switch
                      ...
                      onChange={(enabled) => {
                        if (!enabled) {
                          openModelRetirementModal({
                            config: draft,
                            disabled: false,
                            groupId,
                            onSaved: onMigrationSaved,
                            options: optionsSWR.data || [],
                            requestedSource: JSON.stringify([item.provider, item.model]),
                          });
                          return;
                        }
                        if (
...
ModelDisplayEditor.displayName = 'ModelDisplayEditor';

const ModelDisplaySettings = memo(() => {
  const { t } = useTranslation('setting');
  const [activeGroup, setActiveGroup] = useState('default');
  const [busy, setBusy] = useState(false);
  const groups = useClientDataSWR(
    ['cotti', 'model-management-groups'],
    () => cottiUsersService.groups(),
    { revalidateOnFocus: true },
  );
  if (groups.error) return <AsyncError error={groups.error} onRetry={() => void groups.mutate()} />;
  if (!groups.data) return <Skeleton height={160} />;
  const group = groups.data.find((g) => g.id === activeGroup);
  if (activeGroup !== 'default' && !group)
    return (
      <AsyncError
        error={new Error(t('platformManagement.models.groups.missing'))}
        onRetry={() => setActiveGroup('default')}
      />
    );
  return (
    <Flexbox gap={12}>
      <Tabs
        ...
        items={[
          { key: 'default', label: t('platformManagement.models.groups.default'), disabled: busy },
          ...groups.data.map((g) => ({ key: g.id, label: g.name, disabled: busy })),
        ]}
        onChange={(key) => {
          if (!busy) setActiveGroup(key);
        }}
      />
      {group && (
        <Text type="secondary">
          {t('platformManagement.models.groups.channel', {
            channel: group.provider === 'openai' ? 'Astraflow' : group.provider,
          })}
        </Text>
      )}
      <ModelDisplayEditor groupId={group?.id} key={activeGroup} onBusyChange={setBusy} />
    </Flexbox>
  );
});
