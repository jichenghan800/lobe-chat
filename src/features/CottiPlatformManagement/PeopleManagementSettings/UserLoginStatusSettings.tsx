...
import { Block, Empty, Flexbox, Icon, Input, Tag, Text } from '@lobehub/ui';
  ...
  ({ disabledUsers, onChanged }) => {
    ...
    const searchSWR = useClientDataSWR(
      debouncedQuery.trim().length >= 2
        ...
        : null,
      () => cottiAgentAccessService.searchUsers(debouncedQuery.trim()),
      ...
      <Block className={sharedStyles.card} gap={16} padding={20} variant={'outlined'}>
        ...
        <div className={styles.formGrid}>
          ...
          <AutoComplete
            ...
            value={form.query}
            onChange={(query) => setForm((current) => ({ ...current, query, userId: '' }))}
            onSelect={(userId, option) =>
              setForm((current) => ({ ...current, query: String(option.label), userId }))
            }
          >
            <Input />
          </AutoComplete>
