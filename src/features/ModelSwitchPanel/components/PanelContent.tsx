...
export const PanelContent: FC<PanelContentProps> = ({
  ...
  const chatEnabledList = useEnabledChatModels(modelDisplayScope);
  ...
  const { panelWidth, handlePanelWidthChange } = usePanelSize();

  const { contentHeight, maxListHeight, maxPanelHeight } = getModelSwitchPanelHeightConstraints();
  ...
  if (isDevMode) {
    return (
      <Rnd
        ...
        minWidth={MIN_WIDTH}
        position={{ x: 0, y: 0 }}
        size={{ height: contentHeight, width: panelWidth }}
        style={{
          display: 'flex',
