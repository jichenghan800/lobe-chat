...
describe('scope visibility and global retirement', () => {
  it('keeps an Agent-only Qwen available without exposing it in Chat', () => {
    const model = { model: 'qwen3.8-max-0902', provider: 'qwen' };
    const config: ModelDisplayConfig = {
      agent: [{ ...model, enabled: true }],
      chat: [{ ...model, enabled: false }],
      defaults: { agent: model },
    };
    expect(isModelEnabledInDisplayScope(config, 'agent', model)).toBe(true);
    expect(isModelEnabledInDisplayScope(config, 'chat', model)).toBe(false);
    expect(getModelDisplayDefault(config, 'agent')).toEqual(model);
    const available = [{ id: 'qwen', children: [{ id: model.model }] }];
    expect(applyModelDisplayConfig(available, config.agent)).toEqual(available);
    expect(applyModelDisplayConfig(available, config.chat)).toEqual([]);
    config.retirements = [
      { source: model, target: { model: 'gpt-5.6-terra', provider: 'azure' }, at: '', by: 'admin' },
    ];
    expect(isModelEnabledInDisplayScope(config, 'agent', model)).toBe(false);
    expect(getModelDisplayDefault(config, 'agent')).toBeUndefined();
  });
});
