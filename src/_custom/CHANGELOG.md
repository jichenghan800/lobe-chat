...
- 类型: docs
- 涉及文件: `src/_custom/RELEASE_READY_v2.1.47.md`
- 原因：当前运行环境、数据库、queue 模式与二开能力都已切到 `v2.1.47` 基线之上，但分支与发布语义仍需要一个明确的冻结说明，避免后续把 “已验证 dev 版本” 和 “生产候选版本” 混淆
- 方案：新增发布就绪文档，记录当前提交落点、已纳入的关键二开能力、已完成验证、仍待生产侧补齐的事项，以及 `custom-rebase / db-rehearsal / release` 三条分支的收口方式
- 验证：文档与当前运行态、测试结果、分支状态人工核对一致
- 回滚：删除 `src/_custom/RELEASE_READY_v2.1.47.md`
