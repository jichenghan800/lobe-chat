...
const AUTH_ENABLE_MAGIC_LINK = process.env.AUTH_ENABLE_MAGIC_LINK === '1';
const AUTH_DISABLE_EMAIL_PASSWORD = process.env.AUTH_DISABLE_EMAIL_PASSWORD === '1';
...
const resolveRedirectUrl = (req: NextRequest) => {
  const base = appEnv.APP_URL || req.nextUrl.origin;
...
const buildMagicLinkVerifyUrl = (req: NextRequest, token: string) => {
  const base = appEnv.APP_URL || req.nextUrl.origin;

  let verifyUrl: URL;
  try {
    verifyUrl = new URL('/api/auth/magic-link/verify', base);
  } catch {
    verifyUrl = new URL('/api/auth/magic-link/verify', req.nextUrl.origin);
  }
...
const shouldUseMagicLinkOnly = () => AUTH_ENABLE_MAGIC_LINK || AUTH_DISABLE_EMAIL_PASSWORD;
...
const redirectWithMagicLink = async (req: NextRequest, email: string) => {
  const ctx = await auth.$context;
  ...
  return NextResponse.redirect(buildMagicLinkVerifyUrl(req, token));
};
...
const handleDevLogin = async (req: NextRequest) => {
  ...
  try {
    ...
    if (shouldUseMagicLinkOnly()) {
      return redirectWithMagicLink(req, DEV_BYPASS_EMAIL);
    }
    ...
    try {
      ...
      if (
        ...
        error.message === 'Email and password is not enabled'
      ) {
        return redirectWithMagicLink(req, DEV_BYPASS_EMAIL);
      }
    ...
    const redirectUrl = resolveRedirectUrl(req);
