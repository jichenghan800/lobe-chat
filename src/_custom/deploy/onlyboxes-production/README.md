...
目标 chat.cotticoffee.com，生产机 10.82.70.138。12 核 / 约 46 GiB RAM。仅开启自建沙箱配置，应用仍为 `lobehub:lingshu-v2217-multipart-prod-20260923-r2`，未发布候选整批代码。
...
验证：Python 执行 56，跨调用文件读写成功，公网 HTTPS 200；控制地址 / 元数据 / 宿主 SSH 连接被拒。生产应用容器使用自身新配置签发测试 JIT，真实终端命令返回 HTTP 200 /exitCode 0 / 56。应用 healthy、公网会话接口 200。尚未完成生产账号浏览器全链路沙箱验收；相对导出修复仍属于待发布候选镜像。
...
生产整批候选发布包 `/opt/lobechat-releases/v2217-upload-sandbox-20260923` 已重新生成环境与容器快照，后续发布必须保留本次 OnlyBoxes 配置。Docker 未升级，应另设维护窗口评估。
