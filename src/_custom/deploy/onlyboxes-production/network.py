...
for name, subnet, bridge in [
    ...
    if not exists('docker', 'network', 'inspect', name):
        run('docker', 'network', 'create', *(['--internal'] if name == 'lingshu-onlyboxes-control' else []), '--driver', 'bridge', '--subnet', subnet,
...
# Production host INPUT policy drops bridge traffic; permit only application control requests.
control_rule = ('-i', 'br-obx-control', '-s', '10.251.216.0/24', '-d', '10.251.216.1', '-p', 'tcp', '--dport', '18489', '-j', 'ACCEPT')
if not exists('iptables', '-C', 'INPUT', *control_rule):
    run('iptables', '-I', 'INPUT', '1', *control_rule)
