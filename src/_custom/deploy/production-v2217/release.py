...
R = Path('/opt/lobechat-releases/v2217-20260913')
...
def main():
    ...
    print('Recently active Agent operations:', active, flush=True)
    if not args.execute:
    ...
    try:
        ...
        with (R / 'reconcile.sql').open('rb') as source, (backup / 'migration.log').open('wb') as log:
            run(['docker', 'exec', '-i', PG, 'psql', '-X', '-U', 'paradedb', '-d', 'lobehub',
                 '-v', 'ON_ERROR_STOP=1'], stdin=source, stdout=log, stderr=log)
        compose(True)
