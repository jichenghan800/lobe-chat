...
context=Path('.records/v2217-build')
...
for source,name in [('scripts/serverLauncher/startServer.js','startServer.js'),('scripts/migrateServerDB/errorHint.js','errorHint.js'),('src/_custom/deploy/v2217/Dockerfile','Dockerfile')]:
...
pnpm exec esbuild scripts/migrateServerDB/docker.cjs --bundle --platform=node --format=cjs --external:pg --outfile=.records/v2217-build/docker.cjs
docker build -t "$LINGSHU_IMAGE_TAG" .records/v2217-build
