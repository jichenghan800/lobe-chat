  ...
  const betterAuthMiddleware = async (req: NextRequest) => {
    ...
    if (!isLoggedIn) {
      ...
      if (isProtected) {
        ...
        const requestHostname = resolveRequestHostname(req.headers, req.nextUrl.hostname);
        if (isCottiAiChatHost(requestHostname)) {
          logBetterAuth('Redirecting Cotti AI host directly to the server-side SSO entry');
